...
class _LoginPageState extends State<LoginPage> {
  ...
  signIn(String email, pass) async {
    ...
    if(response.statusCode == 200) {
      ...
      if(jsonResponse != null) {
        ...
        session.setString("token", jsonResponse['token']);

        //check token
        var tokenResponse = checkToken(responseString);
        log('tokenResponse: $tokenResponse');


        //Compobacion de token y navegacion a pagina nueva
        //var responseWord = http.read("http://18.220.60.222:8080/mascotas-login/hello", headers: {"Authorization": responseString});
  ...
  Container buttonSection() {
    return Container(
      ...
      child: RaisedButton(
        onPressed: emailController.text == "" || passwordController.text == ""? null: () {setState(() {
                  _isLoading = true;
                });
